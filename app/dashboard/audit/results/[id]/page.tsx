...
const css = `
  .results-root { font-family: 'Satoshi', sans-serif; color: #f0f0f5; max-width: 1200px; margin: 0 auto; padding: 0 clamp(16px, 4vw, 24px); }
  ...
  .btn-back { display: inline-flex; align-items: center; gap: 8px; padding: 9px 16px; background: #0e0e18; border: 1px solid #1e1e2e; border-radius: 8px; color: #6b6b85; font-family: 'Satoshi', sans-serif; font-size: clamp(11px, 2.5vw, 12px); cursor: pointer; text-decoration: none; transition: all 0.15s; }
  .btn-back:hover { color: #f0f0f5; border-color: #2e2e45; }
  .btn-actions { display: flex; gap: 10px; flex-wrap: wrap; }
  .btn-action { display: inline-flex; align-items: center; gap: 8px; padding: 9px 18px; border-radius: 8px; font-family: 'Satoshi', sans-serif; font-size: clamp(11px, 2.5vw, 12px); cursor: pointer; transition: all 0.15s; }
  ...
  .audit-label { display: inline-flex; align-items: center; gap: 6px; padding: 4px 12px; border-radius: 100px; background: rgba(99,102,241,0.08); border: 1px solid rgba(99,102,241,0.2); color: #a5b4fc; font-size: clamp(9px, 2vw, 10px); text-transform: uppercase; letter-spacing: 0.08em; margin-bottom: 12px; font-family: 'Satoshi', sans-serif; }
  .contract-name { font-family: 'Satoshi', sans-serif; font-size: clamp(24px, 6vw, 36px); font-weight: 800; color: #f0f0f5; letter-spacing: -0.025em; margin-bottom: 8px; word-break: break-word; }
  .contract-meta { font-size: clamp(10px, 2.5vw, 11px); color: #3a3a55; font-family: 'Satoshi', sans-serif; flex-wrap: wrap; gap: 8px; }
  .score-card { background: #0e0e18; border: 1px solid #1e1e2e; border-radius: 14px; padding: clamp(20px, 5vw, 32px); display: flex; align-items: center; gap: clamp(20px, 5vw, 40px); flex-wrap: wrap; margin-bottom: 16px; justify-content: center; }
  ...
  .gauge-score { font-family: 'Satoshi', sans-serif; font-size: clamp(28px, 6vw, 32px); font-weight: 800; color: #f0f0f5; line-height: 1; }
  .gauge-label { font-size: clamp(8px, 2vw, 9px); color: #6b6b85; margin-top: 2px; font-family: 'Satoshi', sans-serif; }
  .risk-chip { display: inline-block; padding: 3px 10px; border-radius: 6px; font-size: clamp(9px, 2vw, 10px); text-transform: uppercase; letter-spacing: 0.08em; margin-top: 8px; font-family: 'Satoshi', sans-serif; white-space: nowrap; }
  ...
  .count-lbl { font-size: clamp(9px, 2vw, 10px); color: #6b6b85; margin-top: 4px; text-transform: uppercase; letter-spacing: 0.06em; font-family: 'Satoshi', sans-serif; }
  .stats-row { display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 12px; margin-bottom: 16px; }
  .stat-card { background: #0e0e18; border: 1px solid #1e1e2e; border-radius: 10px; padding: clamp(12px, 3vw, 16px); text-align: center; }
  .stat-val { font-family: 'Satoshi', sans-serif; font-size: clamp(16px, 4vw, 18px); font-weight: 700; color: #f0f0f5; word-break: break-word; }
  .stat-lbl { font-size: clamp(9px, 2vw, 10px); color: #6b6b85; margin-top: 4px; font-family: 'Satoshi', sans-serif; }
  .section-card { background: #0e0e18; border: 1px solid #1e1e2e; border-radius: 14px; margin-bottom: 16px; overflow: hidden; }
  .section-head { padding: clamp(14px, 4vw, 18px) clamp(16px, 4vw, 24px); border-bottom: 1px solid #1e1e2e; display: flex; align-items: center; gap: 10px; flex-wrap: wrap; cursor: pointer; }
  .section-title { font-family: 'Satoshi', sans-serif; font-size: clamp(13px, 3.5vw, 14px); font-weight: 700; color: #e0e0f0; }
  .section-body { padding: clamp(16px, 4vw, 24px); }
  .summary-text { font-size: clamp(12px, 3vw, 13px); line-height: 1.9; color: #a0a0b8; font-family: 'Satoshi', sans-serif; }
  .verdict-box { margin-top: 16px; padding: 12px 16px; border-radius: 8px; font-size: clamp(11px, 2.5vw, 12px); font-family: 'Satoshi', sans-serif; }
  .thinking-body { padding: clamp(16px, 4vw, 20px) clamp(16px, 4vw, 24px); }
  .thinking-pre { background: #111118; border: 1px solid #1e1e2e; border-radius: 8px; padding: clamp(12px, 3vw, 16px); font-size: clamp(10px, 2.5vw, 11px); line-height: 1.8; color: #6b6b85; white-space: pre-wrap; overflow-x: auto; font-family: 'Satoshi', monospace; max-height: 400px; overflow-y: auto; }
  .findings-list { }
  .finding-row { border-bottom: 1px solid #1e1e2e; }
  .finding-row:last-child { border-bottom: none; }
  .finding-trigger { width: 100%; padding: clamp(12px, 3vw, 16px) clamp(16px, 4vw, 24px); display: flex; align-items: center; justify-content: space-between; gap: 12px; background: none; border: none; color: #f0f0f5; font-family: 'Satoshi', sans-serif; font-size: clamp(11px, 2.5vw, 12px); cursor: pointer; text-align: left; transition: background 0.1s; }
  ...
  .sev-badge { display: inline-flex; align-items: center; gap: 5px; padding: 3px 10px; border-radius: 6px; font-size: clamp(9px, 2vw, 10px); text-transform: uppercase; letter-spacing: 0.06em; border: 1px solid; font-family: 'Satoshi', sans-serif; white-space: nowrap; }
  .line-chip { font-size: clamp(9px, 2vw, 10px); padding: 2px 8px; border-radius: 4px; background: #111118; border: 1px solid #1e1e2e; color: #6b6b85; font-family: 'Satoshi', monospace; white-space: nowrap; }
  .finding-title { font-size: clamp(12px, 3vw, 13px); color: #e0e0f0; font-family: 'Satoshi', sans-serif; word-break: break-word; }
  .finding-detail { padding: clamp(12px, 3vw, 16px) clamp(16px, 4vw, 24px) clamp(16px, 4vw, 24px); background: #080810; border-top: 1px solid #1e1e2e; }
  .detail-label { font-size: clamp(9px, 2vw, 10px); color: #6b6b85; text-transform: uppercase; letter-spacing: 0.08em; margin-bottom: 8px; font-family: 'Satoshi', sans-serif; }
  .detail-text { font-size: clamp(11px, 2.5vw, 12px); color: #a0a0b8; line-height: 1.8; font-family: 'Satoshi', sans-serif; }
  ...
  .empty-state div:first-of-type { font-size: clamp(14px, 4vw, 16px); margin-bottom: 8px; }
  .empty-state div:last-of-type { font-size: clamp(11px, 2.5vw, 12px); }
  .loading-state { display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 400px; gap: 12px; text-align: center; padding: 20px; }
  .spinner { width: clamp(28px, 6vw, 32px); height: clamp(28px, 6vw, 32px); border: 3px solid rgba(99,102,241,0.2); border-top-color: #6366f1; border-radius: 50%; animation: spin 0.7s linear infinite; }
  .spin-sm { width: 14px; height: 14px; border: 2px solid rgba(255,255,255,0.2); border-top-color: #fff; border-radius: 50%; animation: spin 0.7s linear infinite; display: inline-block; }
  .error-container { max-width: 500px; margin: 80px auto; padding: clamp(20px, 5vw, 32px); background: #0e0e18; border: 1px solid rgba(239,68,68,0.2); border-radius: 14px; }
  ...
  if (loading) return (
    <>
      ...
      <div className="loading-state">
        ...
        <span style={{ fontSize: "clamp(11px, 3vw, 12px)", color: "#6b6b85", fontFamily: "'Satoshi', sans-serif" }}>Loading audit results…</span>
      </div>
    </>
  );
    ...
    <>
      ...
      <div className="error-container">
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12, flexWrap: "wrap" }}>
          ...
          <span style={{ fontFamily: "'Satoshi', sans-serif", fontWeight: 700, fontSize: "clamp(14px, 4vw, 16px)", color: "#fca5a5" }}>Error loading audit</span>
        </div>
        <p style={{ fontSize: "clamp(11px, 2.5vw, 12px)", color: "#6b6b85", marginBottom: 20, fontFamily: "'Satoshi', sans-serif" }}>{error || "Audit not found"}</p>
        <button onClick={() => router.push("/dashboard/audit")} style={{ padding: "10px 20px", background: "#6366f1", color: "#fff", border: "none", borderRadius: 8, cursor: "pointer", fontSize: "clamp(11px, 2.5vw, 12px)", fontFamily: "'Satoshi', sans-serif", width: "100%", maxWidth: 200 }}>
          Start New Audit
        </button>
      </div>
    </>
          ...
          <div className="section-body">
            ...
            {report?.deployment_verdict && (
              <div className="verdict-box" style={{ background: riskStyle.bg, border: `1px solid ${riskStyle.border}` }}>
                ...
                <span style={{ fontSize: "clamp(11px, 2.5vw, 12px)", color: "#a0a0b8" }}>{report.deployment_verdict}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="section-card">
          ...
          {findings.length === 0 ? (
            <div className="empty-state">
              ...
              <div style={{ fontFamily: "'Satoshi', sans-serif", fontWeight: 700, color: "#f0f0f5" }}>No Vulnerabilities Found</div>
              <div style={{ fontFamily: "'Satoshi', sans-serif", color: "#6b6b85" }}>Your contract looks secure. Great work!</div>
            </div>
          ) : (
